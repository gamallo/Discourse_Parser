...
##################################################################################
     ...
     ### Regras troca de nomes de funçoes:
	 #if ( $Dep[$head] =~ /^Circ/ && $Tag[$head] =~ /^PRP/) {
#	 if ( $Dep[$head] =~ /^Circ/ && $Lemma[$head] =~ /^(para)/) {
 #            $Dep[$head] = "AdvL";
  #   }
	...
	##Regra da adverbial phrase"
        elsif ($Tag[$head] eq "ADV" || $Args[$head] =~ /coord:adv/) {
        ...
        ##Regra da adjectival phrase"
        elsif ($Tag[$head] eq "ADJ" || $Args[$head] =~ /coord:adj/) {
        ...
        ##Regra da prepositional phrase"
	 ...
	 elsif ($Dep[$head] =~/^BipolarN/ && $Tag[$head] eq "CONJ") {

             $Unit_dep{$head} = "STA-$Dep[$head]" ;
             $Unit_tag{$head} = "nominal" ;
             $Dep[$head] = "P";
             ...
             $Unit_tag{$head} = "fcl" ;
             $Dep[$head] = "P";
	     print STDERR "--Bipolar\n";
       } 

       elsif ($Dep[$head] =~/^(Adjn|Term|Adv)/ && $Tag[$head] eq "VERB") {
       ...
       elsif ($Dep[$head] =~/^(Dobj|Subj|Iobj)/ && $Tag[$head] eq "VERB") {
